...
class ContactRepositoryImpl @Inject constructor(
    ...
    override fun observeContacts(vaultId: String): Flow<List<ContactSummary>> = flow {
        ...
        emitAll(database.contactsDao().observeAllDetailed().map { entities -> entities.map { it.toModel() } })
    }

    override fun observeContactDetails(vaultId: String, contactId: String): Flow<ContactDetails?> = flow {
        val database = vaultDatabaseFactory.getDatabase(vaultId)
    ...
    override suspend fun upsertContact(vaultId: String, contact: ContactSummary) {
        ...
        dao.upsert(contact.toEntity(now))
        syncClassifications(dao, contact.id, contact.tags, contact.folderName, now)
        dao.insertTimeline(
            TimelineEntity(
    ...
    override suspend fun saveContactDraft(vaultId: String, draft: ContactDraft): String {
        ...
        dao.upsert(draft.toEntity(contactId = contactId, createdAt = existing?.createdAt ?: now, now = now))
        syncClassifications(dao, contactId, draft.tags, draft.folderName, now)
        dao.insertTimeline(
            TimelineEntity(
                timelineId = UUID.randomUUID().toString(),
                contactId = contactId,
    ...
    override suspend fun deleteContact(vaultId: String, contactId: String) {
        ...
        dao.upsert(current.copy(isDeleted = true, deletedAt = System.currentTimeMillis(), updatedAt = System.currentTimeMillis()))
    }

    override suspend fun restoreContact(vaultId: String, contactId: String) {
        val dao = vaultDatabaseFactory.getDatabase(vaultId).contactsDao()
        val current = dao.getAnyById(contactId) ?: return
        dao.upsert(current.copy(isDeleted = false, deletedAt = null, updatedAt = System.currentTimeMillis()))
    }
    ...
    override suspend fun deleteNote(vaultId: String, noteId: String) {
        val database = vaultDatabaseFactory.getDatabase(vaultId)
        database.contactsDao().deleteNoteById(noteId)
    }
    ...
    override suspend fun deleteReminder(vaultId: String, reminderId: String) {
        val database = vaultDatabaseFactory.getDatabase(vaultId)
        database.contactsDao().deleteReminderById(reminderId)
    }
    ...
    private suspend fun syncClassifications(
        ...
        folderName?.takeIf { it.isNotBlank() }?.let {
            dao.upsertFolder(FolderEntity(folderName = it.trim(), displayName = it.trim(), iconToken = "folder", colorToken = "blue", createdAt = now))
        }
        dao.deleteCrossRefsForContact(contactId)
        val normalized = tags.map { it.trim() }.filter { it.isNotBlank() }.distinct()
        dao.upsertTags(normalized.map { TagEntity(tagName = it, displayName = it, colorToken = "default", createdAt = now) })
        dao.insertContactTagCrossRefs(normalized.map { ContactTagCrossRef(contactId = contactId, tagName = it) })
    }
